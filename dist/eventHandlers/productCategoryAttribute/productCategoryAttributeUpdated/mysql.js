...
eventExecutor.on('connected', () => {
    eventExecutor.execute(events_1.default.productCategoryAttributeUpdated, function (event, message) {
        return __awaiter(this, void 0, void 0, function* () {
            const { version } = event;
            const { productCategoryAttributeId, productCategoryId, name, note } = event.payload;
            yield makeMysqlClient_1.ProductCategoryAttribute.update({ productCategoryId, name, note, version }, { where: { productCategoryAttributeId, version: version - 1 } });
        });
    });
});
